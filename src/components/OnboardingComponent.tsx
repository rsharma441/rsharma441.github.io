...
interface OnboardingProps {
  ...
  setRecommendations: React.Dispatch<React.SetStateAction<{ rank: number; reason: string }[]>>;
  answers: string[];
  setAnswers: React.Dispatch<React.SetStateAction<string[]>>;
}

const OnboardingIntro: React.FC<OnboardingProps> = ({ recommendations, setRecommendations, answers, setAnswers }) => {
  ...
  useEffect(() => {
    ...
    fetch('/assets/full_albums.csv')
      ...
      .then((data) => {
        ...
        const albumList = parsed.data.map((row) => `${row.album} - ${row.artist}`);
        const uniqueAlbums = Array.from(new Set(albumList));
        const albumListRandom50 = uniqueAlbums.sort(() => 0.5 - Math.random()).slice(0, 150);
  ...
  useEffect(() => {
    ...
    const handleAnswerSubmit = useCallback((event: React.FormEvent | React.KeyboardEvent, answer: string) => {
      event.preventDefault();
      setAnswers((prev) => [...prev, answer]); // Append new answer without duplicates
      setStep((prev) => prev + 1); // Move to the next step
    }, [setAnswers, setStep]);  // Include only necessary dependencies
  ...
  const fetchPersonalizedRecommendations = async (userAnswers: string[]) => {
    try {
      ...
      const response = await fetch('https://backend-winter-pond-4834.fly.dev/get-recommendations/', {
        method: 'POST',
        headers: {
      ...
      {step < 6 && (
        ...
        >
          ...
          {step >= 1 && (
            ...
            >
              {' '}This year, I listened to over <span style={{ color: '#e20c79' }}>240 albums</span>
            </span>
          )}
          {step >= 2 && (
            <span
          ...
          {step >= 3 && (
            ...
            >
              {' '}something I haven’t done in <span style={{ color: '#e20c79' }}>10 years!!</span>
            </span>
          )}
        </div>
      )}
...
{step >= 7 && step <= 9 && (
  <form
    style={{
      opacity: 1,
      animation: step === 7 || step === 8 || step === 9 ? 'fadeIn 1.5s forwards' : 'none',
      textAlign: 'left',
    }}
    onSubmit={(event) => {
      event.preventDefault();
    ...
    {step === 7 && (
      <>
        <p style={{ animation: 'fadeIn 1.5s forwards', opacity: 0 }}>
          Okay, so this year, the AI-guy in me is going to ask you some questions to recomemnd my reviews more meaningfully. Feel free to press Next if you don`&apos;`t care
        </p>
        <p style={{ animation: 'fadeIn 1.5s forwards', opacity: 0 }}>
        ...
        <textarea
          onKeyDown={(event) => {
            if (event.key === 'Enter') {
              ...
              handleAnswerSubmit(event, (event.target as HTMLTextAreaElement).value);
            }
          }}
          style={{
            width: '80%',
...
{step === 10 && (
  ...
  >
    <h2 style={{ animation: 'fadeIn 1.5s forwards', opacity: 0 }}>Thank you!</h2>
    <p style={{ animation: 'fadeIn 1.5s forwards', opacity: 0 }}>
    ...
    <button
      style={{
        animation: 'fadeIn 1.5s forwards',
        opacity: recommendations.length > 0 || answers.every((a) => a.trim() === '') ? 1 : 0.5, // Greyed out only if answers are not skipped and no recommendations
        pointerEvents: recommendations.length > 0 || answers.every((a) => a.trim() === '') ? 'auto' : 'none', // Enable interaction for skipped answers or when recommendations are ready
        backgroundColor: recommendations.length > 0 || answers.every((a) => a.trim() === '') ? '#e20c79' : '#ccc', // Change background color when enabled
        color: '#fff',
        padding: '10px 20px',
        fontSize: '1rem',
        border: 'none',
        borderRadius: '5px',
        cursor: recommendations.length > 0 || answers.every((a) => a.trim() === '') ? 'pointer' : 'not-allowed',
