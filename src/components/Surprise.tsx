...
const Surprise: React.FC<SurpriseProps> = ({ answers, setAnswers }) => {
  ...
  const fetchPersonalizedRecommendation = async () => {
    ...
    try {
      const response = await fetch(
        'https://backend-winter-pond-4834.fly.dev/surprise/get-recommendations/',
        {
          method: 'POST',
          headers: {
  ...
  return (
    <div className="surprise">
      <p className="title">
        {' '}
        <span style={{ fontWeight: 'bold' }}>AI Recommender: </span>Update your
        answers to the prompts below and use this tool to find more personal
        recs from my reviewed albums
      </p>
      <div
